// RGBHinsdale - modified RGBmatrixPanel to handle horizontal scrolling
...
// Height of proportional-width text
...
static RGBHinsdale *activePanel = NULL;

// Code common to both the 16x32 and 32x32 constructors:
void RGBHinsdale::init(uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
  uint8_t sclk, uint8_t latch, uint8_t oe, boolean dbuf) {

  // The meaning of "dbuf" is now whether to allocate a one-column
  // scroll buffer
  boolean scroll_buf = dbuf;
  dbuf = false;
  ...
  // Enable/disable init
  update_enabled = update_enabled_next = true;

  // Scroll init
  initScroll(scroll_buf);

  initTimerDelays();
}

// Constructor for 16x32 panel:
RGBHinsdale::RGBHinsdale(
...
uint16_t RGBHinsdale::Color333(uint8_t r, uint8_t g, uint8_t b) {
  // RRRrrGGGgggBBBbb
...
uint16_t RGBHinsdale::Color444(uint8_t r, uint8_t g, uint8_t b) {
  // RRRRrGGGGggBBBBb
...
uint16_t RGBHinsdale::Color888(uint8_t r, uint8_t g, uint8_t b) {
  return ((r & 0xF8) << 11) | ((g & 0xFC) << 5) | (b >> 3);
}

// REMOVED gamma
// 8/8/8 -> gamma -> 5/6/5
uint16_t RGBHinsdale::Color888(
  uint8_t r, uint8_t g, uint8_t b, boolean gflag) {
  return ((r & 0xF8) << 11) | ((g & 0xFC) << 5) | (b >> 3);
}

uint16_t RGBHinsdale::ColorHSV(
...
void RGBHinsdale::drawPixel(int16_t x, int16_t y, uint16_t c) {
...
void RGBHinsdale::fillScreen(uint16_t c) {
...
uint8_t *RGBHinsdale::backBuffer() {
...
void RGBHinsdale::swapBuffers(boolean copy) {
...
void RGBHinsdale::dumpMatrix(void) {
...
// Enable or disable update.  Set the enabled status.  If we
// are toggling it, wait to make sure the update routine has
// recognized that fact before we return (by busy-waiting)
void RGBHinsdale::enableUpdate(boolean enab) {
  // Do nothing if setting to already-requested value
  if ( enab && update_enabled_next || !enab && !update_enabled_next )
    return;
  // Set the new status
  update_enabled_next = enab;
  // Busy-wait until the timer handler sets
  while ( update_enabled != update_enabled_next )
    ;
}
...
void RGBHinsdale::updateDisplay(void) {

  // Maintain update flags
  boolean update_this_time = update_enabled;
  update_enabled = update_enabled_next;
  if ( !update_this_time )
    return;
  ...
  *latport |= latpin; // Latch data loaded during *prior* interrupt
  ...
  if(++plane >= nPlanes) {      // Advance plane counter.  Maxed out?
    ...
    if(++row >= nRows) {        // advance row counter.  Maxed out?
      ...
      // This is where we do all processing that needs to be done "atomically,"
      // i.e., on a boundary of a full screen refresh
      if ( scroll_flag ) {
        horiz_scroll = horiz_scroll_next;
        scroll_flag = 0;
        // Also in column of scroll data if marked as availalbe
        if ( scroll_data_available ) {
          int16_t y;
          for ( y = 0; y < _height; y++ )
            drawPixel(horiz_scroll, y, scroll_buff[y]);
          scroll_data_available = 0;
        }
      }
    }
  } else if(plane == 1) {
  ...
  *latport &= ~latpin;  // Latch down
  ...
  // Overview of below:
  // if ( not scrolling )
  //     use legacy code untouched
  // else
  //     do two passes

  // Move the macro up and out
  ...
  if ( horiz_scroll == 0 ) {
    // Use original screen-update code w/out any scrolling at all
    ...
    // Do a two pass update setting the pointer to source data
    // based on the horizontal scroll offset.
    uint8_t *ptr_orig = ptr;
    int pass, count, i;
    for ( pass=0; pass < 2; pass++ ) {

      if ( pass == 0 ) {
        ptr = ptr_orig + horiz_scroll;
        count = _width - horiz_scroll;
      }
      else {
        ptr = ptr_orig;
        count = horiz_scroll;
...
// Draw a column of bits at text cursor, in text color, advancing
// to the right.  This supports proportional-width fonts.  If
// foreground and background are equal, then background is "transparent"
// (i.e., not updated)
void RGBHinsdale::drawTextBitColumn(uint8_t colbits) {
  int i;
  for (i=0; i < TEXT_HEIGHT; i++) {
    if ( colbits & (1 << i) ) {
      // Draw foreground color
      drawPixel(cursor_x, cursor_y + i, textcolor);
    }
    else if ( textcolor != textbgcolor ) {
      // Draw background (if not transparent)
      drawPixel(cursor_x, cursor_y + i, textbgcolor);
    }
  }
  // Advance cursor, wrapping at right and at bottom
  cursor_x++;
  if (cursor_x >= WIDTH) {
    cursor_x = 0;
...
// Init scroll params, and maybe the buffer
void RGBHinsdale::initScroll(boolean dbuf) {
  horiz_scroll = 0;
  scroll_flag = 0;
  horiz_scroll_next = 0;
  scroll_data_available = 0;
  // Allocate pixel-column buffer: 2 bytes for a uint16 color x height
  if ( dbuf )
    scroll_buff = (uint16_t *) malloc(2 * _height);
  else
    scroll_buff = 0;
}

// Set a new horizontal scroll offset, and a flag that it has changed
// This flag will be inspected and reset by the update routine
void RGBHinsdale::setScrollOffset(uint8_t offset) {
  horiz_scroll_next = offset;
  scroll_flag = 1;
}

// Set flag that the scroll column of data is available for writing
// The data will be copied in (and the flag reset) the next time
// a scroll-offset change is handled.
void RGBHinsdale::setScrollDataAvailable() {
  scroll_data_available = 1;
}

// Put a full display column of pixels to the scroll column buffer
void RGBHinsdale::drawPixelToScrollColumn(int16_t y, uint16_t c) {
  if ( scroll_buff )
    scroll_buff[y] = c;
}

// Set the four timer delays
void RGBHinsdale::setTimerDelays(uint16_t * delays) {
  for (int i = 0; i < 4; i++ )
    timer_delays[i] = delays[i];
}

// Init the default timer delays
void RGBHinsdale::initTimerDelays() {
  // Timer delays by plane.  Default to behavior of old code:
  // #define CALLOVERHEAD 60   // Actual value measured = 56
  // #define LOOPTIME     200  // Actual value measured = 188
  // t = (nRows > 8) ? LOOPTIME : (LOOPTIME * 2);
  // duration = ((t + CALLOVERHEAD * 2) << plane) - CALLOVERHEAD;
  if ( nRows > 8 ) {
    // 32x32
    timer_delays[0] = 260;  // ((200 + 60 * 2) << 0) - 60 = 1 * 320 - 60 = 260
    timer_delays[1] = 580;  // ((200 + 60 * 2) << 1) - 60 = 2 * 320 - 60 = 580
    timer_delays[2] = 1220; // ((200 + 60 * 2) << 2) - 60 = 4 * 320 - 60 = 1220
    timer_delays[3] = 2500; // ((200 + 60 * 2) << 3) - 60 = 8 * 320 - 60 = 2500
  }
  else {
    // 16x32
    timer_delays[0] =  460; // ((400 + 60 * 2) << 0) - 60 = 1 * 520 - 60 = 460
    timer_delays[1] =  980; // ((400 + 60 * 2) << 1) - 60 = 2 * 520 - 60 = 980
    timer_delays[2] = 2020; // ((400 + 60 * 2) << 2) - 60 = 4 * 520 - 60 = 2020
    timer_delays[3] = 4100; // ((400 + 60 * 2) << 3) - 60 = 8 * 520 - 60 = 4100
  }
}
